...
panel_install() {
    ...
    local NEED_PHP=false NEED_COMPOSER=false NEED_NODE=false NEED_YARN=false
    ...
    if [[ "$NEED_REDIS" == "true" ]]; then
        case "$OS_ID" in
            ubuntu|debian)     _run "Installing Redis" bash -c "$PKG_INSTALL redis-server" ;;
            almalinux|rocky)   _run "Installing Redis" bash -c "$PKG_INSTALL redis" ;;
        esac
        _run_no_fail "Starting Redis" bash -c \
        ...
        case "$OS_ID" in
            ubuntu|debian)
                ...
                _run "Installing PHP 8.3" bash -c \
                    "$PKG_INSTALL php8.3 php8.3-{cli,gd,mysql,pdo,mbstring,tokenizer,bcmath,xml,fpm,curl,zip,redis}" ;;
            almalinux|rocky)
    ...
    if [[ "$NEED_NODE" == "true" ]]; then
        case "$OS_ID" in
            ubuntu|debian)
                _run "Adding NodeJS 20 repo" bash -c "curl -fsSL https://deb.nodesource.com/setup_20.x | bash -"
                _run "Installing NodeJS"     bash -c "$PKG_INSTALL nodejs" ;;
            almalinux|rocky)
                _run "Adding NodeJS 20 repo" bash -c "curl -fsSL https://rpm.nodesource.com/setup_20.x | bash -"
                _run "Installing NodeJS"     bash -c "$PKG_INSTALL nodejs" ;;
        esac
    fi
    [[ "$NEED_YARN" == "true" ]] && _run "Installing Yarn" bash -c "npm install -g yarn"
    ...
    mysql -u root <<-MYSQL
        ...
        FLUSH PRIVILEGES;
MYSQL
    ui::status "Database + user created" "ok"; log::ok "Database configured"

    # ── Download Panel ────────────────────────────────────────
    ui::subsection "Downloading Pterodactyl Panel"
    local PANEL_VERSION
    PANEL_VERSION=$(curl -fsSL --max-time 10 \
...
UNIT
    ...
    [[ -f "${PANEL_DIR}/public/index.php" ]] \
        && ui::status "Panel files present" "ok" || ui::status "Panel files present" "fail"
    [[ -f "${PANEL_DIR}/.env" ]] \
        && ui::status ".env configured" "ok" || ui::status ".env configured" "fail"
