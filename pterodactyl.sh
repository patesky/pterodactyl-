...
# Pre-fill the menu choice (and the "proceed to wings?" prompt for options 2/5),
# then hand the terminal back to the user so every subsequent installer prompt
# (FQDN, email, database passwords, etc.) is answered interactively.
if [[ "$CHOICE" == "2" || "$CHOICE" == "5" ]]; then
    { printf '%s\nY\n' "$CHOICE"; cat /dev/tty; } | bash <(curl -s https://pterodactyl-installer.se)
else
    { printf '%s\n' "$CHOICE"; cat /dev/tty; } | bash <(curl -s https://pterodactyl-installer.se)
fi
